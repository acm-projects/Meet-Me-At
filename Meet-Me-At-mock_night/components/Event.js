...
export default function Event({ info, onPress, style}){
    ...
    //console.log("event");
    const date = new Date(event.raw.time.start.year, event.raw.time.start.month - 1, event.raw.time.start.day)
    ...
    <Text style={[globalStyles.SFProText_Heavy, {fontSize: 24, color: '#fff'}]}>{event.startDate.split("/")[1]}</Text>
                <Text style={[globalStyles.SFProText_Regular, {fontSize: 12, color: '#fff'}]}>{ days[dayName] }</Text>
            </View>
